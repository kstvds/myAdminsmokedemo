...
public class General_details_about_rates_can_be_edited {
	...
	Logger logger = Logger.getLogger("General_details_about_rates_can_be_edited");
	
	 @Test
	 @Parameters({ "browsername" })
	  public void editrategeneraldetails(String browsername) throws Exception {
		  test = rep.startTest("editrategeneraldetails");
			...
			test.log(LogStatus.FAIL, "Login");
	           ...
	           try {
					logger.info("Navigating to ContractList");
					   ...
					   driverqa.findElement(RateFilter.travelstartdate).sendKeys(excel.getData(1, 7, 0));
					   Thread.sleep(2000);
					   obj.Takesnap(driverqa, Config.SnapShotPath() + "/Rates/General_details_about_rates_can_be_edited/Final-Rate-filter-screen.jpg");
					   ...
					   Thread.sleep(2000);
					   obj.Takesnap(driverqa, Config.SnapShotPath() + "/Rates/General_details_about_rates_can_be_edited/Contract-List-screen.jpg");
	           ...
	           try {
					logger.info("Editing general details");
					   ...
					   Thread.sleep(2000);
					   obj.Takesnap(driverqa, Config.SnapShotPath() + "/Rates/General_details_about_rates_can_be_edited/Before_edit.jpg");
					   driverqa.findElement(EditRates.editratenotes).clear();
					   driverqa.findElement(EditRates.editratenotes).sendKeys(excel.getData(1, 10, 0));
					   Thread.sleep(2000);
					   obj.Takesnap(driverqa, Config.SnapShotPath() + "/Rates/General_details_about_rates_can_be_edited/After_edit.jpg");
					   driverqa.findElement(EditRates.savenotechanges).click();
					   wait.until(ExpectedConditions.visibilityOfElementLocated(EditRates.notesupdated));
					   obj.Takesnap(driverqa, Config.SnapShotPath() + "/Rates/General_details_about_rates_can_be_edited/Success.jpg");
					   String actualrateresult= driverqa.findElement(EditRates.notesupdated).getText();
					   String rateexpectedtitle = "The rate was updated successfully";
					   Assert.assertEquals(actualrateresult, rateexpectedtitle);
					   logger.info("Edited general details");
					   test.log(LogStatus.PASS, "Edited general details");
				} catch (Exception e) {
					logger.info(e.getMessage());
					test.log(LogStatus.FAIL, e.getMessage());
					rep.endTest(test);
