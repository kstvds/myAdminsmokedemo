...
import ObjectRepository.CancelAccom;
...
public class Accommodation_booking_cancel_reservation {
	...
	Logger logger = Logger.getLogger("Accommodation_booking_cancel_reservation");
	
	 @Test
	  @Parameters({ "browsername" })
	  public void cancel(String browsername) throws Exception {
		  test = rep.startTest("cancel");
		...
		logger.info("Searching Customer");
	         ...
	         try {
				driverqa.findElement(NewAccoBooking.roomtypes).click();
				 ...
				 obj.Takesnap(driverqa, Config.SnapShotPath() + "/Accommodation_booking_cancel_reservation/Selected-room-available.jpg");
				 Thread.sleep(1000);
				 ...
				 driverqa.findElement(NewAccoBooking.paxFname).sendKeys(excel.getData(2, 14, 0));
				 Thread.sleep(2000);
				 driverqa.findElement(NewAccoBooking.paxLname).sendKeys(excel.getData(2, 14, 1));
				 ...
				 Thread.sleep(3000);
				 obj.Takesnap(driverqa, Config.SnapShotPath() + "/Accommodation_booking_cancel_reservation/Passenger-Details.jpg");
				 ...
				 driverqa.findElement(PaymentPage.ccNum).sendKeys(excel.getData(2, 18, 0));
				 driverqa.findElement(PaymentPage.cvv).sendKeys(excel.getData(2, 18, 1));
				 driverqa.findElement(PaymentPage.ccName).sendKeys(excel.getData(2, 18, 2));
				 Select expirydate = new Select(driverqa.findElement(PaymentPage.expYear));
				 expirydate.selectByIndex(2);
				 driverqa.findElement(PaymentPage.zip).sendKeys(excel.getData(2, 14, 2));
				 driverqa.findElement(PaymentPage.termsconditions).click();
				 Thread.sleep(2000);
				 obj.Takesnap(driverqa, Config.SnapShotPath() + "/Accommodation_booking_cancel_reservation/Card-Details.jpg");
				 ...
				 obj.Takesnap(driverqa, Config.SnapShotPath() + "/Accommodation_booking_cancel_reservation/After-booking.jpg");
	         ...
	         try {
				test.log(LogStatus.INFO, "Cancelling BookedHotel");
				 logger.info("Cancelling BookedHotel");
				 wait.until(ExpectedConditions.visibilityOfElementLocated(CancelAccom.cancelbutton));
				 driverqa.findElement(CancelAccom.cancelbutton).click();
				 wait.until(ExpectedConditions.visibilityOfElementLocated(CancelAccom.confirmcancel));
				 Thread.sleep(2000);
				 String expectedcanceltitle = "Applicable cancellation charges";
				 String atualcanceltitle = driverqa.findElement(CancelAccom.confirmcancelstatus).getText();
				 Assert.assertTrue(atualcanceltitle.contains(expectedcanceltitle));
				 obj.Takesnap(driverqa, Config.SnapShotPath() + "/Accommodation_booking_cancel_reservation/Confirm-Cancel.jpg");
				 driverqa.findElement(CancelAccom.confirmcancel).click();
				 String expectedcancellationstatus="Confirmed";
				 wait.until(ExpectedConditions.visibilityOfElementLocated(CancelAccom.cancelstatus));
				 String actualcancellationstatus= driverqa.findElement(CancelAccom.cancelstatus).getText();
				 Thread.sleep(5000);
				 obj.Takesnap(driverqa, Config.SnapShotPath() + "/Accommodation_booking_cancel_reservation/After-Cancel.jpg");
				 Assert.assertTrue(actualcancellationstatus.contains(expectedcancellationstatus));
				 test.log(LogStatus.PASS, "PASSED Cancel_Booking");
